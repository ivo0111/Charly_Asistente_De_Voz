...
EXTRA_CONTEXT = """Eres un asistente de voz llamado Charly. Responde en español, de forma natural y breve.
...
1. prueba(number: float) -> float: Devuelve el resultado de aplicar la función Maurística al número dado.
2. hora_actual() -> str: Devuelve la hora y fecha actual en formato legible.
3. abrir_programa(nombre: str) -> str: Abre un programa o sitio web en Windows.
4. buscar_wikipedia(consulta: str, oraciones: int = 2) -> str: Busca un resumen en Wikipedia en español.
5. obtener_clima(ciudad: str) -> str: Devuelve el clima actual de la ciudad especificada.

Después de recibir el resultado, úsalo para continuar la conversación naturalmente.
...
- No anticipes los resultados de las herramientas, espera a recibirlos antes de continuar la conversación.
- Al escribir, evita signos de puntuación innecesarios como "¡" o "¿"
- al usar números, escribe los números en palabras (ejemplo: "dos" en lugar de "2")
"""
...
def procesar_respuesta(texto_usuario):
    ...
    if "<mcp>" in respuesta and "</mcp>" in respuesta:
        try:
            ...
            respuesta_final = generar_respuesta(nueva_pregunta)
